...
# UoW)Только зарегистрированный пользователь может вести дневник
...
#     if (len(exercise.emotions) >= UoW and len(exercise.events) >= UoW) and (exercise.date < (datetime.datetime.now() + datetime.timedelta(days=UoW))):
...
#
# a1 = Achievement(UoW, datetime.datetime(2003, 11, 28), 'Главное достижение')
...
#
# U1 = User(id=UoW, login=f"{fake.word()}", gender='male', password="123456789")
...
#
# d1 = DiaryMoods(UoW, U1, [ex1, ex2], [a2, a3], ['Никогда не сдавайся'])
